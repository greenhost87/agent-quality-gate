...
const pluginPath = resolve(import.meta.dir, '../../../oxlint/dao-boundaries.ts');
const fixturesRoot = resolve(import.meta.dir, '../../../.quality-fixtures');

async function writeRuleConfig(rule: string) {
  const workspace = realpathSync(mkdtempSync(join(tmpdir(), 'database-oxlint-')));
  const configPath = join(workspace, 'oxlint.json');
  await write(
    configPath,
    JSON.stringify({
      categories: { correctness: 'off' },
      jsPlugins: [{ name: 'database', specifier: pluginPath }],
      rules: { [rule]: 'error' },
    }),
  );
