...
const mixedFeaturesOptions = {
  directories: ['app/components/features'],
  maxDepth: {
    'app/components/features': 2,
  },
};

test('module placement allows mixed flat and nested feature modules at maxDepth 2', async () => {
  for (const entry of [
    'app/components/features/fabrics/decimal-field.tsx',
    'app/components/features/fabrics/card/card.tsx',
    'app/components/features/fabrics/card/details.tsx',
  ] as const) {
    const result = await runOxlintFixture(
      'module-placement/valid/mixed-features',
      entry,
      mixedFeaturesOptions,
    );
    expect(result.output).toBe('');
    expect(result.status).toBe(0);
  }
