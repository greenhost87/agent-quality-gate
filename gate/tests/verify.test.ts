...
describe('verify', () => {
  ...
  it('rejects oxlint-disable directives via baseline oxlint rule', async () => {
    const cwd = await createTypeScriptProject('forbidden-directive/src/index.ts');
    const result = await runVerify(cwd);
    const output = result.stdout + result.stderr;

    expect(result.exitCode).toBe(1);
    expect(output).toContain('aqg(no-oxlint-disable-directives)');
    expect(output).toContain('Inline lint directives are forbidden');
  });
  ...
  it('merges project ignorePatterns into the generated Fallow config during verify', async () => {
    const cwd = await createTypeScriptProject('clean-function/src/index.ts');
    ...
    expect(result.exitCode).toBe(0);
    expect(existsSync(join(cwd, '.aqg', 'fallow'))).toBe(false);
    expect(existsSync(join(cwd, '.aqg', 'oxlint'))).toBe(false);
  });
