  ...
  while (Date.now() < deadline) {
    try {
      ...
      const records = content
        ...
        .flatMap((line) => {
          const result = v.safeParse(StatsRecordSchema, JSON.parse(line));
...
describe('verify run stats', () => {
  it('appends a JSONL record after executeVerify without changing the result', async () => {
    ...
    expect(timings.pl).toBeGreaterThanOrEqual(0);
    expect(timings.o).toBeGreaterThanOrEqual(0);
    expect(timings.sh).toBeGreaterThanOrEqual(0);
    expect(timings.cx).toBeGreaterThanOrEqual(0);
  ...
  it('records early validation failures', async () => {
    ...
    expect(record.pl).toBeUndefined();
