...
- Local `bun run outdated` (`scripts/outdated/outdated.ts`) runs `bun outdated` in the repository and every preset pack with a lockfile; `bun run outdated -- --update` runs `bun update --latest` in the same roots;
